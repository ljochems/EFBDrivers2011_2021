...
#setwd("/home/ljochems/FullVeg_CV/")
...
for (k in seq_len(final_dfs)) {
  z <- final_dfs[[k]]$hyd_bin
  ...
  stack.EFB_y <- inla.stack(data = list(alldata = cbind(y,NA)), 
                            ...
                            effects = list(s.index_mY = spde$n.spde,
                                         list(b0Y = rep(1, nrow(final_dfs[[k]])),
                                              data.frame(depth=scale(final_dfs[[k]]$wtr__)[,1]),data.frame(typha=scale(final_dfs[[k]]$typ_cover)[,1]),
                                              data.frame(boats=scale(final_dfs[[k]]$NEAR_DIST)[,1]), data.frame(fetch=scale(final_dfs[[k]]$MeanFetch)[,1]),
                                              idY = rep(1,nrow(final_dfs[[k]])), idY2 = rep(1,nrow(final_dfs[[k]])),idY3 = rep(1,nrow(final_dfs[[k]])),
                                              idY4 = rep(1,nrow(final_dfs[[k]])))), 
                            tag = "Beta (EFB Cover)")
  ...
  stack.EFB_z <- inla.stack(data = list(alldata = cbind(NA,z)), 
                            ...
                            effects = list(s.index_mZ = spde$n.spde,
                                         list(b0Z = rep(1, nrow(final_dfs[[k]])),
                                              data.frame(depth=scale(final_dfs[[k]]$wtr__)[,1]),data.frame(typha=scale(final_dfs[[k]]$typ_cover)[,1]),
                                              data.frame(boats=scale(final_dfs[[k]]$NEAR_DIST)[,1]), data.frame(fetch=scale(final_dfs[[k]]$MeanFetch)[,1]),
                                              idZ = rep(1,nrow(final_dfs[[k]])), idZ2 = rep(1,nrow(final_dfs[[k]])),idZ3 = rep(1,nrow(final_dfs[[k]])),
                                              idZ4 = rep(1,nrow(final_dfs[[k]])))), 
  ...
  save(EFB.hurdlemodel.inla.complete, file = paste0("/home/ljochems/FullVeg_CV/ModelFold_",k,".RData"))
