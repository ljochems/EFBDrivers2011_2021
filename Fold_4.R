...
train_dfs <- lapply(train_list, function(i) efb_fine[match(i,efb_fine$OID),]) 
test_dfs <- lapply(test_list, function(i) efb_fine[match(i,efb_fine$OID),])
...
z <- final_dfs[[4]]$hyd_bin
y <- ifelse(final_dfs[[4]]$EFB_cover > 0, yes=final_dfs[[4]]$EFB_cover,no = NA)
y_noNA <- y[!is.na(y)]
cens <- 0.001
...
stack.EFB_y <- inla.stack(data = list(alldata = cbind(y,NA)), 
                          ...
                          effects = list(s.index_mY = spde$n.spde,
                                         list(b0Y = rep(1, nrow(final_dfs[[4]])),
                                              data.frame(depth=scale(final_dfs[[4]]$wtr__)[,1]),data.frame(typha=scale(final_dfs[[4]]$typ_cover)[,1]),
                                              data.frame(boats=scale(final_dfs[[4]]$NEAR_DIST)[,1]), data.frame(fetch=scale(final_dfs[[4]]$MeanFetch)[,1]),
                                              idY = rep(1,nrow(final_dfs[[4]])), idY2 = rep(1,nrow(final_dfs[[4]])),idY3 = rep(1,nrow(final_dfs[[4]])),
                                              idY4 = rep(1,nrow(final_dfs[[4]])))), 
                          tag = "Beta (EFB Cover)")

stack.EFB_z <- inla.stack(data = list(alldata = cbind(NA,z)), 
                          A = list(A_matrix, 1),
                          effects = list(s.index_mZ = spde$n.spde,
                                         list(b0Z = rep(1, nrow(final_dfs[[4]])),
                                              data.frame(depth=scale(final_dfs[[4]]$wtr__)[,1]),data.frame(typha=scale(final_dfs[[4]]$typ_cover)[,1]),
                                              data.frame(boats=scale(final_dfs[[4]]$NEAR_DIST)[,1]), data.frame(fetch=scale(final_dfs[[4]]$MeanFetch)[,1]),
                                              idZ = rep(1,nrow(final_dfs[[4]])), idZ2 = rep(1,nrow(final_dfs[[4]])),idZ3 = rep(1,nrow(final_dfs[[4]])),
                                              idZ4 = rep(1,nrow(final_dfs[[4]])))), 
...
save(EFB.hurdlemodel.inla.complete, file = "Fold4.RData")
