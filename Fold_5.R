...
z <- final_dfs[[5]]$hyd_bin
y <- ifelse(final_dfs[[5]]$EFB_cover > 0, yes=final_dfs[[5]]$EFB_cover,no = NA)
y_noNA <- y[!is.na(y)]
cens <- 0.001
...
stack.EFB_y <- inla.stack(data = list(alldata = cbind(y,NA)), 
                          ...
                          effects = list(s.index_mY = spde$n.spde,
                                         list(b0Y = rep(1, nrow(final_dfs[[5]])),
                                              data.frame(depth=scale(final_dfs[[5]]$wtr__)[,1]),data.frame(typha=scale(final_dfs[[5]]$typ_cover)[,1]),
                                              data.frame(boats=scale(final_dfs[[5]]$NEAR_DIST)[,1]), data.frame(fetch=scale(final_dfs[[5]]$MeanFetch)[,1]),
                                              idY = rep(1,nrow(final_dfs[[5]])), idY2 = rep(1,nrow(final_dfs[[5]])),idY3 = rep(1,nrow(final_dfs[[5]])),
                                              idY4 = rep(1,nrow(final_dfs[[5]])))), 
                          tag = "Beta (EFB Cover)")

stack.EFB_z <- inla.stack(data = list(alldata = cbind(NA,z)), 
                          A = list(A_matrix, 1),
                          effects = list(s.index_mZ = spde$n.spde,
                                         list(b0Z = rep(1, nrow(final_dfs[[5]])),
                                              data.frame(depth=scale(final_dfs[[5]]$wtr__)[,1]),data.frame(typha=scale(final_dfs[[5]]$typ_cover)[,1]),
                                              data.frame(boats=scale(final_dfs[[5]]$NEAR_DIST)[,1]), data.frame(fetch=scale(final_dfs[[5]]$MeanFetch)[,1]),
                                              idZ = rep(1,nrow(final_dfs[[5]])), idZ2 = rep(1,nrow(final_dfs[[5]])),idZ3 = rep(1,nrow(final_dfs[[5]])),
                                              idZ4 = rep(1,nrow(final_dfs[[5]])))), 
